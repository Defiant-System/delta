...
Impact.Entity = Impact.Class.extend({
	id: 0,
	settings: {},
	...
	update: function() {
		...
		this.vel.y += Impact.game.gravity * Impact.system.tick * this.gravityFactor;
		
		this.vel.x = this.getNewVelocity( this.vel.x, this.accel.x, this.friction.x, this.maxVel.x );
		this.vel.y = this.getNewVelocity( this.vel.y, this.accel.y, this.friction.y, this.maxVel.y );
		
		// movement & collision
		var mx = this.vel.x * Impact.system.tick;
		var my = this.vel.y * Impact.system.tick;
		var res = Impact.game.collisionMap.trace( 
...
Impact.Entity.checkPair = function( a, b ) {
	...
	if(
		...
		a.collides + b.collides > Impact.Entity.COLLIDES.ACTIVE
	) {
		Impact.Entity.solveCollision( a, b );
	}
};


Impact.Entity.solveCollision = function( a, b ) {
	
	// If one entity is FIXED, or the other entity is LITE, the weak
	// (FIXED/NON-LITE) entity won't move in collision response
	var weak = null;
	if(
		a.collides == Impact.Entity.COLLIDES.LITE ||
		b.collides == Impact.Entity.COLLIDES.FIXED
	) {
		weak = a;
	}
	else if(
		b.collides == Impact.Entity.COLLIDES.LITE ||
		a.collides == Impact.Entity.COLLIDES.FIXED
	) {
		weak = b;
	}
	...
	if(
		...
		if (a.last.y < b.last.y ) {
			...
			Impact.Entity.seperateOnYAxis( b, a, weak );
		}
		a.collideWith( b, 'y' );
		b.collideWith( a, 'y' );
	}
	...
	// Horizontal collision
		...
		if (a.last.x < b.last.x ) {
			...
			Impact.Entity.seperateOnXAxis( b, a, weak );
		}
		a.collideWith( b, 'x' );
		b.collideWith( a, 'x' );
	}
...
Impact.Entity.seperateOnXAxis = function( left, right, weak ) {
	...
	if (weak ) {
		...
		var resWeak = Impact.game.collisionMap.trace( 
	...
	// Normal collision - both move
		...
		var resLeft = Impact.game.collisionMap.trace( 
			left.pos.x, left.pos.y, -nudge/2, 0, left.size.x, left.size.y
		);
		left.pos.x = Math.floor(resLeft.pos.x);
		
		var resRight = Impact.game.collisionMap.trace( 
...
Impact.Entity.seperateOnYAxis = function( top, bottom, weak ) {
	...
	// Normal collision - both move
		...
		var nudgeX = bottom.vel.x * Impact.system.tick;
		var resTop = Impact.game.collisionMap.trace( 
			top.pos.x, top.pos.y, nudgeX, -nudge/2, top.size.x, top.size.y
		);
		top.pos.y = resTop.pos.y;
		
		var resBottom = Impact.game.collisionMap.trace( 
